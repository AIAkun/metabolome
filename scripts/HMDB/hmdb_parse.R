hmdb_parse <- function(xmlName)
{
  ...
  dat <- as.data.frame(t(sapply(hmdb, "[", i = 1:max(sapply(hmdb, length)))))
  list_cols <- sapply(dat, function(x) is.list(x))
  dat[list_cols] <- lapply(dat[list_cols], function(x) sapply(x, paste, collapse = ";")) 
  write.csv(dat,"hmdbtaxonomy.csv",row.names = F)
}
library(XML)
hmdb_parse('hmdb_metabolites.xml')